...
assert.deepEqual(loadSharedCameraFraming(storage), { theta: 0, phi: 75, fov: 45, verticalShift: 0 });
assert.equal(saveSharedCameraFraming({ theta: 22.5, phi: 60, fov: 37, verticalShift: 0.25 }, storage), true);
assert.deepEqual(loadSharedCameraFraming(storage), { theta: 22.5, phi: 60, fov: 37, verticalShift: 0.25 });
assert.equal(saveSharedCameraFraming({ theta: 999, phi: -2, fov: 200, verticalShift: -9 }, storage), true);
assert.deepEqual(loadSharedCameraFraming(storage), { theta: 180, phi: 1, fov: 90, verticalShift: -0.5 });
...
assert.deepEqual(loadSharedCameraFraming(storage), { theta: 10, phi: 80, fov: 50, verticalShift: 0 });
